...
class hexPosition (object):
    ...
    def machine_vs_machine_SilentMode(self, machine1=None, machine2=None):
        """
        Let two AIs play a game against each other silently.
        The variables machine1 and machine2 must point to a function that maps a board state and an action set to an element of the action set.
        If a machine is not specified, random actions will be used.
        This method prints only the name of the winning agent.
        """
        # Use random players as default if machines are not specified
        if machine1 is None:
            def machine1(board, action_list):
                from random import choice
                return choice(action_list)

            machine1_name = "RandomAgent1"
        else:
            machine1_name = machine1.__name__
        ...
        if machine2 is None:
            ...
            machine2_name = "RandomAgent2"
        else:
            machine2_name = machine2.__name__
        ...
        if self.winner == 1:
            print(f"The winner is {machine1_name}")
        elif self.winner == -1:
            print(f"The winner is {machine2_name}")
